class Thorax.Views.PatientBankView extends Thorax.Views.BonnieView
  ...
  updateToggledPatient: (result) ->
    if result
      associatedDifference = @differences.filter (d) -> _(d.result.patient).isEqual result.patient
      @$('[data-model-cid="'+associatedDifference[0].cid+'"]').find("[data-toggle='collapse']").click()
      @toggledPatient = associatedDifference.result
    else
      @toggledPatient = null

  updateSelectedPatients: (patients) ->
    associatedDifferences = @differences.filter (d) -> patients.contains d.result.patient
