class PatientsController < ApplicationController
  ...
  def export
    ...
    stringio = Zip::ZipOutputStream::write_buffer do |zip|
      ...
      elsif (params[:results] && params[:patients])
        zip.put_next_entry("bank_patients_results.html")
        zip.puts bank_patients_summary(records, params[:results].values, qrda_errors, html_errors)
      end
    end
    cookies[:fileDownload] = "true" # We need to set this cookie for jquery.fileDownload
    stringio.rewind
    send_data stringio.sysread, :type => 'application/zip', :disposition => 'attachment', :filename => "bonnie_patient_export.zip"
  end
...
private
  ...
  def update_patient(patient)
    ...
    ['first', 'last', 'gender', 'expired', 'birthdate', 'description', 'description_category', 'deathdate', 'is_shared'].each {|param| patient[param] = params[param]}
  ...
  def bank_patients_summary(patients, results, qrda_errors, html_errors)
    patients.each do |patient|
      # add the measure name for each patient so it can be rendered in the summary
      patient_measure_title = get_associated_measure(patient).pluck(:cms_id, :title)[0]
      patient['patient_measure_title'] = "#{patient_measure_title[0]}: #{patient_measure_title[1]}"
    end
    results = output_differences(results)
    render_patient_summary(patients, results, qrda_errors, html_errors, 'downloaded_patients')
  end

  def measure_patients_summary(patients, results, qrda_errors, html_errors, measure)
    results = output_differences(results)
    render_patient_summary(patients, results, qrda_errors, html_errors, 'index', measure)
  end

  def output_differences(results)
  ...
  def render_patient_summary(records, results, qrda_errors, html_errors, templatename, *measure)
    ...
    rendering_context.render(:template => templatename, :locals => {records: records, results: results, qrda_errors: qrda_errors, html_errors: html_errors, measure: measure[0]})
  end
